...
class moveit_handle:
    ...
    def set_robot_perpendicular(self):
        """ Rotate robot perpendicular to shelf """
        ...
        if not -114 > rot_yaw > -105: ## the range where the package face is considered to be perpendicular to robot
            self.set_rotate_angle_pub.publish(110 + rot_yaw)
            time.sleep(3)
            self.log("Aligning {}".format(110 + rot_yaw))
            return False
        else:
    ...
    def decrease_y_distance(self, delta_y):
        ...
        angle = Int32(85) if delta_y > 0 else Int32(-85)
        self.set_rotate_angle_pub.publish(angle)
        time.sleep(6)
        rospy.loginfo("Spin 90")
        self.set_move_distance_pub(Float32(delta_y))
        time.sleep(3)
    ...
    def move_to_shelf(self):
        ...
        while not self.set_robot_perpendicular() :
            timeout = rospy.get_rostime().secs - time_start 
            if not self.spin_joint_0_finding(self.shelf_id) or timeout > 30:
                return
        self.group.set_named_target("ready_1")
        self.handle_command(1)
            
        ### turn robot parallel to shelf and move to center the robot with shelf code
        start_yaw = self.yaw_angle_deg
        ...
        for i in range(7):
            ...
            rospy.loginfo("trans y : {}".format(trans_.y))
            self.set_robot_perpendicular()
            if abs(trans_.y) > 0.05:
                self.decrease_y_distance(trans_.y + 0.03)
                continue

            trans_.x += 0.5 * trans_.y
            self.log("Transform_shelf_2: {} \n {}".format(trans_, rot_))
            if 0.3 <= trans_.x <= 0.38:
                self.log("In the working range")
                break
            elif trans_.x > 0.38:
                time_delay = abs(trans_.x - 0.38) / 0.13
                speed = 0.14
            elif trans_.x < 0.3:
                time_delay = abs(trans_.x - 0.3) / 0.13
                speed = -0.14
                if self.__is_back_obstacle:
                    continue
            self.speed_pub.publish(Float32(speed))
            self.log("Time delay: {}".format(time_delay))
